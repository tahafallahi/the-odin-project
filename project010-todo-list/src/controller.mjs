...
class Controller {
    ...
    this.view.bindCloseBtn()
        ...
        this.view.bindToDoList((btn) => this.#handleDeleteToDoList(btn), (toDoObj, id) => this.#handleEditToDoForm(toDoObj, id));
    }

    #renderPage() {
        const projectArray = this.model.projects;
        if (projectArray) this.view.renderProjects(projectArray, this.#selectedProjectIndex);
    }

    #handleProjects(id) {
        if (!id) throw new Error("Invalid Id");
        let targetProjectObj = this.model.projects.find(project => project.id == id);
