...
describe("Project and ToDo classes", () => {
  ...
  test("should not allow duplicate ToDo in a Project", () => {
    project.addToDo(todo);
    expect(() => project.addToDo(todo)).toThrow(
      "Todo objects must be unique in each project.",
    );
  });
  ...
  test("should throw error if updating non-existent ToDo", () => {
    expect(() => project.updateToDo("fake-id", { title: "nope" })).toThrow(
      "Can not update non-existent todo object",
    );
  });
