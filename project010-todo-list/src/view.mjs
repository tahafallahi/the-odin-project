...
export default class View {
    ...
    bindToDoList(deleteHandler, editHandler){
        this.#toDoListElm.addEventListener("click", e => {
            if (e.target.id == "delete-note") {
                ...
                dialog.showModal();
                this.bindEditToDoForm(editHandler, e.target.parentElement.parentElement.id)
            }
        })
    }
    ...
    bindCreateProject() {
        ...
        btn.addEventListener("click", () => {
            ...
            btn.parentElement.insertBefore(this.#pseudoTextbox, btn.parentElement.firstChild);
...
bindEditToDoForm(handler, id) {
    ...
    this._editListener = (event) => {
        event.preventDefault();
        const toDoForm = new FormData(event.target);
        handler(Object.fromEntries(toDoForm), id);
        dialog.close();
    };
